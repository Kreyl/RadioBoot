...
hdr.write("*/\n\n")
hdr.write("#define flash_upd(dst, src, size)                   ((FLASH_UPD_TYPE)((unsigned int)__FLASH_UPD + 1))(dst, src, size)\n")
hdr.write("#define flash_upd_sram(sram_func, dst, src, size)   ((FLASH_UPD_TYPE)((unsigned int)sram_func + 1))(dst, src, size)\n\n")
hdr.write("#define FLASH_UPD_SIZE                   " + str(len(h) / 2) + "\n\n")
hdr.write("extern const uint8_t __FLASH_UPD[FLASH_UPD_SIZE];\n\n")
hdr.write("typedef int (*FLASH_UPD_TYPE)(unsigned int, unsigned int, int);\n\n")
